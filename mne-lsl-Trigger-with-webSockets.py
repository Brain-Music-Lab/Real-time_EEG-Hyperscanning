...
## Load the clean data
root = "D:/Documents/CurrentStudy/"
P_root = "TriggerTest/sub-P001/ses-S001/eeg/"
clean = "sub-P001_ses-S001_task-test_run-001_eeg.xdf"
...
async def send_array(websocket, epochs_lead, epochs_follow, M_l, T_l, M_f, T_f, BaselineConn, nChan, sfreq, freq_bands):

    print("Starting to send data over WebSocket...")
    ...
    while True:
        ...
        HypClenEpochs = np.stack([clean_l, clean_f])  # shape: (2, 5, 16, 750)
        ...
        matrixMask[nChan:2*nChan, 0:nChan] = conn_coh[nChan:2*nChan, 0:nChan]
...
async def main():
    async with websockets.serve(
        lambda ws, path: send_array(ws, epochs_lead, epochs_follow, M_l, T_l, M_f, T_f, BaselineConn, nChan, sfreq, freq_bands),
        "localhost",
        8080
    ) as server:
